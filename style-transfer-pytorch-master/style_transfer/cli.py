...
def save_image(path, image, message=False):
    path = Path(path)
    if message: tqdm.write(f'Writing image to {path}.')
...
class Callback:
    ...
    def __call__(self, iterate):
        ...
        if iterate.i == iterate.i_max:
            ...
            save_image(self.args.output, self.st.get_image(self.image_type), False)
